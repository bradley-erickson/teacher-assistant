...
export async function insertScore(studentID,classID,total,correct,dateStamp){
    let result = [];
    await fetch('/insertScore', {method: 'post',
                        headers: {'Accept': 'application/json, text/plain, */*','Content-Type': 'application/json'},
                        body: JSON.stringify({studentID: studentID, classID:classID,total:total,correct:correct,dateStamp:dateStamp})
                      })
        .then(res => res.json())
        .then(response => result.push(response));
    return result;
